...
static ACTIVE_WORDS: AtomicUsize = AtomicUsize::new(0);
...
pub fn init(device_count: usize) {
    ...
    ACTIVE_WORDS.store((next_slot + 63) / 64, Relaxed);
}
...
pub fn query_avg_f64(target: CompType, filter: CompType) -> (f64, usize) {
    ...
    for w in 0..active_words() {
        let mask =
            COMP_PRESENCE[target.index()].load_word(w) & COMP_PRESENCE[filter.index()].load_word(w);
...
pub fn query_avg_f64_group_v1(target: CompType, filter: CompType, group: usize) -> (f64, usize) {
    ...
    for w in 0..active_words() {
        let mask = COMP_PRESENCE[target.index()].load_word(w)
            & COMP_PRESENCE[filter.index()].load_word(w)
            & GROUP_SCOPE[group].load_word(w);
...
pub fn query_avg_f64_device_v1(target: CompType, filter: CompType, device: usize) -> (f64, usize) {
    ...
    for w in 0..active_words() {
        let mask = COMP_PRESENCE[target.index()].load_word(w)
            & COMP_PRESENCE[filter.index()].load_word(w)
            & DEVICE_SCOPE[device].load_word(w);
...
impl BitSet {
    const fn new() -> Self {
        Self {
            words: [const { AtomicU64::new(0) }; BITSET_WORDS],
        }
    }
    ...
    fn set_bit(&self, index: usize) {
        let word = index / 64;
        let bit = index % 64;
        self.words[word].fetch_or(1 << bit, Relaxed);
    }

    fn clear_all(&self, num_words: usize) {
...
pub fn reset() {
    let words = ACTIVE_WORDS.load(Relaxed);
...
mod tests {
    ...
    #[test]
    fn v1_device_dimmable() {
        ensure_init();
        let (avg, count) = query_avg_f64_device_v1(CompType::Dimmer, CompType::Light, 1);
        assert_eq!(count, 1);
        assert!((avg - 0.75).abs() < f64::EPSILON);
    }

    #[test]
    fn v1_device_no_dimmer() {
        ensure_init();
        let (avg, count) = query_avg_f64_device_v1(CompType::Dimmer, CompType::Light, 2);
        assert_eq!(count, 0);
        assert_eq!(avg, 0.0);
    }
    ...
    #[test]
    fn v2_device_no_dimmer() {
        ensure_init();
        let (avg, count) = query_avg_f64_device_v2(CompType::Dimmer, CompType::Light, 2);
        assert_eq!(count, 0);
        assert_eq!(avg, 0.0);
    }
    ...
    #[test]
    fn versions_agree_global_real() {
        ensure_init();
        let v1 = query_avg_f64(CompType::Real, CompType::Sensor);
        let (expected_avg, expected_count) = expected_real_with_sensor(N);
        assert_eq!(v1.1, expected_count);
        assert!((v1.0 - expected_avg).abs() < 1e-10);
    }
