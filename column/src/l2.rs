...
pub fn query_avg_f64(target: CompType, filter: CompType) -> (f64, usize) {
    ...
    let mut summary = COMP_PRESENCE[target.index()].load_summary()
        & COMP_PRESENCE[filter.index()].load_summary();
...
pub fn query_avg_f64_group_v1(target: CompType, filter: CompType, group: usize) -> (f64, usize) {
    ...
    let mut summary = COMP_PRESENCE[target.index()].load_summary()
        & COMP_PRESENCE[filter.index()].load_summary()
        & GROUP_SCOPE[group].load_summary();
...
pub fn query_avg_f64_device_v1(target: CompType, filter: CompType, device: usize) -> (f64, usize) {
    ...
    let mut summary = COMP_PRESENCE[target.index()].load_summary()
        & COMP_PRESENCE[filter.index()].load_summary()
        & DEVICE_SCOPE[device].load_summary();
...
impl BitSet {
    ...
    fn set_bit(&self, index: usize) {
        ...
        self.summary.fetch_or(1u64 << word, Relaxed);
    }

    fn clear_all(&self) {
        let mut s = self.summary.load(Relaxed);
        while s != 0 {
            let w = s.trailing_zeros() as usize;
            self.layer0[w].store(0, Relaxed);
            s &= s - 1;
        }
        self.summary.store(0, Relaxed);
    }
